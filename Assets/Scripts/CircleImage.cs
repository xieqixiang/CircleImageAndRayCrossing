...
public class CircleImage : BaseImage
{
    ...
    private void RayCrossing(Vector2 p, List<Vector3> vertices, ref int crossNumber)
    {
        ...
        for (int i = 0, count = vertices.Count; i < count; i++)
        {
            ...
            //if (p.y < Mathf.Min(v1.y, v2.y))
            //{ //如果目标点低于线段，跳过
            //    continue;
            //}

            //if (p.y >= Mathf.Max(v1.y, v2.y))
            //{ //如果目标点高于这个线段，跳过
            //    continue;
            //}

            double slope = (double)(v2.y - v1.y) / (double)(v2.x - v1.x); //斜率

            bool cond1 = (v1.x <= p.x) && (p.x < v2.x);
            bool cond2 = (v2.x <= p.x) && (p.x < v1.x);

            bool above = (p.y < slope * (p.x - v1.x) + v1.y);
            if ((cond1||cond2) && above) {
                crossNumber++;
            }
        }

        Debug.Log("asdfsd ="+crossNumber);
    }
}
